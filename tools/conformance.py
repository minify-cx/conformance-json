...
def validate_identity(payload, expected_commit=None):
 ...
 # A Minify++ result must carry name/version/version_string/commit with a
 # well-formed 40-hex commit (and, in the release workflow, the pinned commit);
 # every suite's oracle must carry its required non-empty fields.
